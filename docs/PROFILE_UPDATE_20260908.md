...
本页记录根目录 `data/` 与 `tables/` 的来源画像更新，早于冻结 benchmark v1 的完整日构造。当前预测数据见 [数据构造](DATA_CONSTRUCTION.md)。

SGSC 16,342 条记录和 iFlex 2,071 条记录都已加入扩展画像；候选成员、历史读数、目标电量、活动条件和已有参考估计保持原值。

逐列复核发现汇总表遗漏完整设备列表，以及部分筛选回答尚未进入画像，现已修正并重建两份数据。具体差异、仍暂存追溯区的字段及核对范围见 [提取复核](EXTRACTION_AUDIT_20260908.md)。
